...
plot_as_function <- function(mousedata, 
  ...
  plt +
    annotate('rect', xmin = 0, xmax = window_size$width, ymin = 0, ymax = window_size$height, 
  ...
  if(ntrials == 0){
    plt + annotate(geom = 'text', x = window_size$width/2, y = window_size$height/2, 
                   label = 'No Trials', 
...
df.tidy %>%
  ...
  mutate(response_prod = case_when(response_stim == product1 ~ 'Prod1',
                                   ...
                                   response_stim == product3 ~ 'Prod3')) -> df.sorted
